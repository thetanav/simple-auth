...
import { signinSchema, signupSchema } from "../zod/zod.types.ts";
import { prisma } from "../lib/prisma.ts";
import type { JwtAuthPayload } from "../types/signup.ts";
...
authRouter.get("/session", auth, async (req, res) => {
...
authRouter.get("/logout-session", auth, async (req, res) => {
    // this route is to remotely logout a session with session id

    if (req.user.userId !== "5f8444ae-fa36-4615-8d65-ed42b098e6d6") {
    ...
    try {
        const exist = await prisma.session.findUnique({
            where: { id: sessionId },
        });
        if (!exist) {
        ...
        await prisma.$transaction([
            prisma.token.deleteMany({ where: { sessionId } }),
            prisma.session.delete({ where: { id: sessionId } }),
        ]);
        return res
            .status(200)
            .json({ message: "Session logged out successfully" });
    } catch (error) {
...
authRouter.post("/forgot-password", auth, async (req, res) => {
...
authRouter.post("/reset-password", auth, async (req, res) => {
